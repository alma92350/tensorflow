...
def run(train_data, target_data, voca, len_voca):
    ...
    if int(options.MODEL_NUM) == 0:
        ...
        with graph.as_default():
            x = tf.placeholder("float", [None, max_len, len_voca], name='x')
            y = tf.placeholder("float", [None, len_voca], name='y')
            weight = tf.get_variable("weight",shape=[num_units, len_voca],dtype=tf.float32)
            bias = tf.get_variable("bias",shape=[len_voca],dtype=tf.float32)
            '''
            define rnn cell and prediction
            '''
            rnn_x = tf.transpose(x, [1, 0, 2])
            rnn_x = tf.reshape(rnn_x, [-1, len_voca])
            rnn_x = tf.split(rnn_x, max_len, 0)

            cell = tf.contrib.rnn.BasicLSTMCell(num_units, forget_bias=1.0)
            outputs, states = tf.contrib.rnn.static_rnn(cell, rnn_x, dtype=tf.float32)
            prediction = tf.add(tf.matmul(outputs[-1], weight), bias)
            # define saver for start from scratch
            ...
            print("Initialized bias:", bias.eval(session=sess))
    ...
    for i in range(epoch):
        print("----------- Epoch {0}/{1} -----------".format(i+1+model_num, epoch+model_num))
        count = 0
        ...
        gs = i+1+model_num
        saver.save(sess, 'textgen/my_test_model',global_step=gs)
        # Generates MetaGraphDef.
        saver.export_meta_graph('textgen/my_test_model-' + str(gs) + '.meta')

        print("bias", bias.eval(session=sess))
